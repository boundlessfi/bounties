...
export function useBountySubscription() {
  ...
  useGraphQLSubscription<BountyCreatedData>(
    BOUNTY_CREATED_SUBSCRIPTION,
    {},
    () => {
      invalidateAllLists();
  ...
  useGraphQLSubscription<BountyUpdatedData>(
    BOUNTY_UPDATED_SUBSCRIPTION,
    {},
    (data) => {
      queryClient.invalidateQueries({
        queryKey: bountyKeys.detail(data.bountyUpdated.id),
      });
      invalidateAllLists();
    },
  );
  ...
  useGraphQLSubscription<BountyDeletedData>(
    BOUNTY_DELETED_SUBSCRIPTION,
    {},
    (data) => {
      queryClient.removeQueries({
        queryKey: bountyKeys.detail(data.bountyDeleted.id),
      });
      invalidateAllLists();
    },
  );
