...
export function SubmissionApprovalPanel({
  ...
  const handleRequestRevisions = () => {
    ...
    // Check if we have a submissionId from submissions array
    const submissionId = bounty.submissions?.[0]?.id;

    if (!submissionId) {
      toast.error("No active submission found to request revisions on");
      return;
    }
    ...
    requestRevisions(
      ...
      },
      {
        ...
        onError: (err) => {
          toast.error(
            `Failed to request revisions: ${err instanceof Error ? err.message : "Unknown error"}`,
          );
        },
      },
    );
            ...
            {!showRevisionForm ? (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1 mb-2">
                    Explain what needs to be changed for approval.
                  </p>
                  <Textarea
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {isRequesting ? "Sending..." : "Submit Feedback"}
                  </Button>
                </div>
              </div>
            )}
